...
namespace partiality{
  ...
  struct prediction_ellipse{
    ...
    const panel* p;               // all we need to know about panel
    ...
    double inline pred(const double fs,const double& ss){
      return exp(-0.5*(operator()(fs,ss)+a2))*sqrt(det(iS))/sqrt(4*pi*pi);
    }
    double inline part(const double& fs,const double& ss){
      return exp(-0.5*(operator()(fs,ss)+a2));
    }
    double inline proj(const double fs,const double& ss){
  ...
  struct parameters{
    ...
    source_summand const inline average_source() const {
      ...
      double mean_wvn = 0;
      matrix<double,3,3> M2_div{0,0,0};
      matrix<double,3,3> div{0,0,0};
      double M2_bnd = 0;
      double bnd = 0;
...
*/
  ...
  matrix<double,3,1> const inline optimize_wout(
    ...
    const matrix<double,3,3> iLS= inv_lower_triangular(chol(S));
    //const matrix<double,3,3> iS = inv(S);
    const matrix<double,3,3> iS = inv(S);
    //const matrix<double,3,3> iS = iLS*iLS;
    ...
    //const matrix<double,3,3> PiSPpwtw = P*iS*P+wtw;
    //const matrix<double,3,3> tmp= inv_lower_triangular(chol(PiSPpwtw));
    const matrix<double,3,3> iPiSPpwtw = pinv(P*iS*P+wtw);
    const matrix<double,3,1> Dx = 
      //inv(P*iS*P+wtw)*(P*iS*(win+m/wvn-w));
      //                           /pow(wvn,2u);
      //inv(P*iS*P+wtw)*P*inv(S)*(w-m/wvn-win);
      //cholesky_decomposition<matrix<double,3,3>>(P*
      //    cholesky_decomposition<matrix<double,3,3>>(S).solve(P)+wtw).solve(P*
      //    cholesky_decomposition<matrix<double,3,3>>(S).solve(w-m/wvn-win));
      iPiSPpwtw*(P*iS*(w-m/wvn-win));
      //inv(P*iS*P+wtw)*P*iS*(P*w-d);
      //S*P*cholesky_decomposition<matrix<double,3,3>>(S).solve(m/wvn+win-w);
      //S*P*iS*(m/wvn+win-w);
    double v = pow(wvn,2u)*trans(w-m1)*iS*(w-m1);
    if (o) cout << v << endl;
    for (double c=1.0;c>1e-8;c*=exp(-1)){
    ...
    tuple<
      ...
      vector<
        tuple<
          ...
          vector<tuple<double,prediction_ellipse>>
        >
      >
    >
  >
  const inline get_prediction_ellipses_(
    const parameters& parm,
    const geometry::geometry& geom,
    const double min_partiality = 1.0/64
    ){
    ...
    cout << "calculating qrange" << endl;
    const auto qrange = minmaxq(average_source.win,geom);
    cout << "qrange = [ " << get<0>(qrange) << ", "
                          << get<1>(qrange) << " ]" << endl; 
    cout << "estimating maximum wave number " << endl;
    const double max_f = estimate_max_f(average_source,min_partiality);
    cout << "maximum wave number = " << max_f << endl;
    cout << parm.crystl.size() << " discrete crystal orientations" << endl;
    const matrix<int32_t,3,1> ehkl = extent_hkl(
      minmaxq(average_source.win,geom),get<1>(parm.crystl[0]),max_f);
    cout << "extent of hkl:" << endl;
    cout << ehkl << endl;
    ...
    for(int32_t l=-ehkl(2);l<=ehkl(2);++l)
    for(int32_t k=-ehkl(1);k<=ehkl(1);++k)
    for(int32_t h=-ehkl(0);h<=ehkl(0);++h){
      ...
      for (size_t j=0;j!=parm.crystl.size();++j){
        ...
        //outofrange&=
        //  (length(ms[j])/max_f>get<1>(qrange))
        //  ||(length(ms[j])>5);
      }
      //for (size_t j=0;j!=parm.crystl.size();++j){
      //  if (length(ms[j])>3.5) outofrange=true;
      //}
      outofrange = false;
      ...
      for (size_t j=0;j!=parm.crystl.size();++j){
        ...
        const matrix<double,3,1> Dw   = wouts[j]-average_source.win;
        const matrix<double,3,1> m1   = average_source.wvn*Dw-ms[j];
        const double e = 
          abs(0.5*trans(m1)
              *inv(constant_covariance_matrix(Dw,ms[j],
                                              average_source,
                                              get<1>(parm.crystl[j])))*m1);
        partiality+=e>LOG_DBL_MAX?0:get<0>(parm.crystl[j])*exp(-e);
        sumw+=get<0>(parm.crystl[j]);
      }
      partiality/=sumw;
      // partiality with average source and constant covariance matrix
      // cout << "conservative partiality:" << partiality << endl;
      if ( partiality < pow(min_partiality,2u) ) continue; // be conservative
      partiality = 0;
      sumw = 0;
      ...
      for (size_t j=0;j!=parm.crystl.size();++j){
        ...
        matrix<double,3,1>& wout     = wouts[i*parm.crystl.size()+j];
        const matrix<double,3,1>& win= source.win;
        wout = normalize(ms[j]+kin);
        if (o) cout << trans(wout);
        ...
        for (size_t n=0;n!=10;++n){
          const matrix<double,3,1> m1 = wout-ms[j]/wvn-win;
          const matrix<double,3,3> S    =
            covariance_matrix(wout-win,ms[j],source,crystl);
          const matrix<double,3,3> iS   = inv(S);
          const double e = pow(wvn,2u)*trans(m1)*iS*m1; 
          if (o) cout << e << endl;
          bool success = true;
          const matrix<double,3,1> tmp =
            optimize_wout(wout,ms[j],source,crystl,success);
          wout = tmp;
          if (!success) break;
          if (o) cout << trans(wout);
        }
        ...
        //  wouts[i],ms[j],source,crystl))*(source.wvn*wouts[i]-ms[j])) << endl;
        const matrix<double,3,1> m1 = wout-ms[j]/wvn-win;
        const double w = source_weight*crystl_weight;
        const matrix<double,3,3> S    =
          covariance_matrix(wout-win,ms[j],source,crystl);
        const matrix<double,3,3> iS   = inv(S);
        if (o) cout << trans(m1);
        const double e = pow(wvn,2u)*abs(0.5*trans(m1)*iS*(m1));
        if (o) cout << e << endl;
        partiality+=e>LOG_DBL_MAX?0:w*exp(-e);
        sumw += w;
      }
      partiality/=sumw;
      if ( partiality < min_partiality) continue;
      //cout << "partiality = " << partiality << endl;
      //cout << trans(hkl);
      //cout << "###############################################################"
      //     << endl;
      unordered_map<const geometry::panel*,
        vector<tuple<double,prediction_ellipse>>> subellipse_map;
      for (size_t i=0;i!=parm.source.size();++i)
      for (size_t j=0;j!=parm.crystl.size();++j){
        //cout << "---------------" << endl;
        const double& source_weight  = get<0>(parm.source[i]);
        ...
        const matrix<double,3,1>& win= source.win;
        const matrix<double,3,3> S   =
          covariance_matrix(w-source.win,ms[j],source,crystl);
        const matrix<double,3,3> iS = inv(S);
...
//   *  matrix D *   * x *     * offset *      * y *
        ...
        //cout << P << endl;
        const matrix<double,3,1> m0= source.wvn*(P*x+source.win-y/norm_y)+ms[j];
        //cout << m0 << endl;
        ...
        // TODO should not need abs here TODO c2 is fucked :(
        // const double c2 = abs(trans(m0)*iS*m0-trans(m1)*iS1*m1);
        const double c2 = pow(wvn,2u)*
          abs(trans(w-ms[j]/wvn-win)*iS*(w-ms[j]/wvn-win));
        S1+=matrix<double,2,2>{0.5,0,0,0.5}; // cspad with oversampling
        iS1=inv(S1);
        const double weight = source_weight*crystl_weight;
        //cout << c2 << endl;
        //cout << trans(x) << endl;
        //cout << trans(m1) << S1; // TODO m1 is fuckd
        subellipse_map[p].push_back(tuple<double,prediction_ellipse>{weight,
          prediction_ellipse{x,S1,iS1,c2,1.0/(2.0*pi*trans(w)*S*w),p}});
      }
      if (subellipse_map.size()==0) continue;
      vector<tuple<const geometry::panel*,
        vector<tuple<double,prediction_ellipse>>>> subellipses;
        ...
        for (auto crystl=parm.crystl.begin();crystl!=parm.crystl.end();++crystl){
          ...
          const matrix<double,3,3> S =
            covariance_matrix(
                ...
                average_source,
                get<1>(parm.crystl[0]));
          const double max_e = 3*sqrt(-2*log(min_partiality)); // this is agressive
          const double eh = max_e*sqrt(S(0,0));
          const double ek = max_e*sqrt(S(1,1));
          const double el = max_e*sqrt(S(2,2));
          const int32_t lo_h = floor(m(0)-eh); // but it will suffice because
          const int32_t hi_h =  ceil(m(0)+eh); // here we round up and down
          const int32_t lo_k = floor(m(1)-ek); // respectively providing the
          const int32_t hi_k =  ceil(m(1)+ek); // necessary buffer in almost
          const int32_t lo_l = floor(m(2)-el); // all cases, in rare circumstances
          const int32_t hi_l =  ceil(m(2)+el); // it can get very close though
