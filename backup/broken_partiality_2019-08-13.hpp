...
namespace partiality{
  ...
  struct pixel_value{
    double flx;  // flux
    double wvn;  // wavenumber = 1/lambda
    double bnd;  // bandwidth = σ_{wvn}
    crystl dflx; // derivative of flux wrt. crystal
    crystl dwvn; // derivative of wavenumber wrt. crystal
    crystl dbnd; // derivative of bandwidth wrt. crystal
  };

  struct prediction_element{
    int32_t h,k,l;
    double flx;
    double wvn;
    double bnd;
    double lorentz;
    matrix<double,2,1> fsss;
    matrix<double,2,2> shape;
    vector<pair<size_t,pixel_value>> peak;
  };

  template<size_t verbosity=0,size_t oversampling=1>
  vector<prediction_element>
