...
namespace partiality{
  ...
  struct source{
    ...
    const double width_squared(const matrix<double,3,1> w) const {
      return pow(bnd*(1-pow(trans(w)*win)),2) + div;
    }
    const double width(const matrix<double,3,1>& w) const {
      return sqrt(width_squared(w));
    }
    const double area() const {
      return 4*pi*pow(wvn,2);
    }
    // volume of ewald sphere of given width 
    const double volume(const double& width) const {
      return sqrt(2*pi*pow(width,2))*area();
    }
  };
  ...
  tuple<double,source> const inline average_source(
    ...
    return {sumw,source{mean_win,mean_wvn,bnd,div}};
  }
  ...
  bool const inline is_candidate(
      const IDX hkl,
      const tuple<double,source>& source,
      const struct crystl& crystl,
  ...
  template<size_t verbosity>
  double const inline partiality(
      const matrix<double,3,1>& x,
      const matrix<double,3,3>& P,
      const tuple<double,source>& source
      )
  {
    // project x to ewald sphere
    const matrix<double,3,1> p   = x*(wvn/length(x));
    const matrix<double,3,3> S_o = P + S;
    // locally approximate ewald sphere as planar
    return get<0>(source)*exp(-trans(x-p)*SP*(x-p))/;
  }
